...
//                3.0; /// cage radius 1
  ...
  inline void recordReward(Reward *rewards) {
    ...
    rewForwardVel_ = exp(-(bodyLinearVel_.head(1) / (bodyLinearVel_.head(2).norm() + 1e-5)  - targetVector.head(1)).squaredNorm() / 0.25); // std::min(0.5, (gv_.head(2) - targetVector.head(2)*0.5).norm());
